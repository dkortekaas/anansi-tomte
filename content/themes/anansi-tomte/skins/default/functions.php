...
if ( ! function_exists ( 'logiqShop_currency_language' ) ) :
function logiqShop_currency_language() {
    global $pvc_Options;
    ...
    <?php if(isset($pvc_Options['enable_header_currency']) && ($pvc_Options['enable_header_currency']!=0)) : ?>
        <div class="dropdown block-currency-wrapper">
            <a role="button" data-toggle="dropdown" data-target="#" class="block-currency dropdown-toggle" href="#">
                <?php esc_attr_e('USD', 'pvc');?> <span class="caret"></span>
            </a>
            <ul class="dropdown-menu" role="menu">
...
if ( ! function_exists ( 'logiqShop_msg' ) ) :
function logiqShop_msg() { 
    global $pvc_Options;
...
if ( ! function_exists ( 'logiqShop_logo_image' ) ) {
...
if ( ! function_exists ( 'logiqShop_mobile_search' ) ) {
function logiqShop_mobile_search()
{ global $pvc_Options;
  $MagikPvc = new MagikPvc();
    if (isset($pvc_Options['header_remove_header_search']) && !$pvc_Options['header_remove_header_search']) : 
        echo'<div class="mobile-search">';
         echo magikPvc_search_form();
         echo'<div class="search-autocomplete" id="search_autocomplete1" style="display: none;"></div></div>';
         endif;
}
}

if ( ! function_exists ( 'logiqShop_search_form' ) ) :
function logiqShop_search_form() {
    global $pvc_Options;
    $MagikPvc = new MagikPvc();
    ?>
    ...
    <form name="myform" method="GET" action="<?php echo esc_url(home_url('/')); ?>">
        ...
        <?php endif; ?>
        <button type="submit" title="<?php esc_attr_e('Search', 'pvc'); ?>" class="search-btn-bg">
                ...
                <div id="rev_slider_6_1" class="rev_slider fullwidthabanner tp-overflow-hidden" style="display:none;" data-version="5.0.5">
                  <ul>
                   <?php foreach ($pvc_Options['home-page-slider'] as $slide) : ?>
                               ...
                               <li data-transition="3dcurtain-horizontal,3dcurtain-vertical,cube,cube-horizontal" data-slotamount="7,7,7,7" data-masterspeed="600,600,600,600" data-thumb='<?php echo esc_url($slide['thumb']); ?>'>
                                       <img
...
function magikPvc_home_offer_banners()
{
  ...
  if ($pvc_Options['enable_home_offer_banners'] && $pvc_Options['enable_home_offer_banners']!=''){
        ?>
        <!-- banner -->
 <div class="promotion-banner">
    ...
    // Mailchimp Signup
    if ( ! function_exists ( 'wl_footer_signupform' ) ) :
        function wl_footer_signupform() {
            ...
            <div class="home-block-inner">
              ...
              <?php if (isset($pvc_Options['bestseller_image']) &&  !empty($pvc_Options['bestseller_image']['url']))
                  ...
                  <a href="<?php echo !empty($pvc_Options['bestseller_product_url']) ? esc_url($pvc_Options['bestseller_product_url']) : '#' ?>">
                    ...
                    alt="<?php esc_attr_e('bestseller_image', 'pvc'); ?>">
                  </a>  
...
?>
  ...
  <div class="daily-deal-section">
    <div class="container">
      <div class="row">
        <div class="col-sm-6 col-xs-12 hot-deal-img">
            <?php if (isset($pvc_Options['daily_deal_image']) && !empty($pvc_Options['daily_deal_image']['url'])) 
            {?>    

           <a href="<?php echo !empty($pvc_Options['daily_deal_url']) ? esc_url($pvc_Options['daily_deal_url']) : '#' ?>" title="<?php esc_attr_e('daily-deal', 'pvc'); ?>" class="product-image">
                <img src="<?php echo esc_url($pvc_Options['daily_deal_image']
                 ['url']); ?>" alt="<?php esc_attr_e('daily-deal', 'pvc'); ?>"> 
               </a>
            <?php } ?>
    ...
    if ( ! function_exists ( 'magikPvc_newproduct_template' ) ) :
        function magikPvc_newproduct_template() {
            ...
            if (has_post_thumbnail()) :
                $imageUrl =  wp_get_attachment_image_src(get_post_thumbnail_id(),'wl-product-size-large');
            endif; ?>
                            ...
                            <div class="box-hover">
                                <ul class="add-to-links">
                                    ...
                                    <?php if (class_exists('YITH_WCQV_Frontend')) { ?>
                                        <a class="yith-wcqv-button link-quickview" href="#" data-product_id="<?php echo esc_html($product->id); ?>"><?php esc_attr_e('Quick View', 'pvc'); ?></a>
                                    <?php } ?>
                                    </li>
                                    <li>
                                    <?php
                                    ...
                                    ?>
                                        <a href="<?php echo esc_url($yith_wcwl->get_addtowishlist_url()) ?>"  data-product-id="<?php echo esc_html($product->id); ?>" data-product-type="<?php echo esc_html($product->product_type); ?>" <?php echo htmlspecialchars_decode($classes); ?>>
                                            <?php esc_attr_e('Wishlist', 'pvc'); ?>
                                        </a>
                                    <?php } ?>
                                    </li>
                                    <li>
                    ...
                    <div class="item-info">
                        <div class="info-inner">
                            <div class="item-title">
                                <a href="<?php the_permalink(); ?>" title="<?php echo htmlspecialchars_decode($post->post_title); ?>"> <?php echo htmlspecialchars_decode($post->post_title); ?> </a>
                            </div>
...
<?php
    ...
    if ( ! function_exists ( 'wl_related_upsell_template' ) ) :
        function wl_related_upsell_template() {
            //$MagikPvc = new MagikPvc();
            global $product, $woocommerce_loop, $yith_wcwl, $post;
            $imageUrl = woocommerce_placeholder_img_src(); ?>
            <div class="item">
                        ...
                        <div class="item-img-info">
                            <a href="<?php the_permalink(); ?>" title="<?php echo htmlspecialchars_decode($post->post_title); ?>" class="product-image">
                                ...
                                <?php if (has_post_thumbnail()) :
                                    echo wp_get_attachment_image(get_post_thumbnail_id(), apply_filters('magikPvc-product-size-large'));
                                else : ?>
                                    <img alt="<?php echo htmlspecialchars_decode($post->post_title); ?>" src="<?php echo esc_url($imageUrl[0]); ?>">
                                <?php endif; ?>
                                </figure>
                            ...
                            <div class="box-hover">
                                <ul class="add-to-links">
                                    ...
                                    <?php
                                        if (isset($yith_wcwl) && is_object($yith_wcwl)) :
                                            $classes = get_option('yith_wcwl_use_button') == 'yes' ? 'class="link-wishlist"' : 'class="link-wishlist"'; ?>
                                            <a href="<?php echo esc_url($yith_wcwl->get_addtowishlist_url()) ?>"  data-product-id="<?php echo esc_html($product->id); ?>" data-product-type="<?php echo esc_html($product->product_type); ?>" <?php echo htmlspecialchars_decode($classes); ?>><?php esc_attr_e('Wishlist', 'pvc'); ?></a>
                                    <?php endif; ?>
                                    </li>
                                    <li>
                                    <?php if (class_exists('YITH_Woocompare_Frontend')) :
                                        $mgk_yith_cmp = new YITH_Woocompare_Frontend; ?>
                                        <a href="<?php echo esc_url($mgk_yith_cmp->add_product_url($product->id)); ?>" class="compare link-compare add_to_compare" data-product_id="<?php echo esc_html($product->id); ?>"><?php esc_attr_e('Compare', 'pvc'); ?></a>
                                    <?php endif; ?>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div class="item-content">
                                <div class="item-price">
                                    ...
                                    if ( $product->max_variation_price && $product->max_variation_price !== $product->min_variation_price ) :
                                        echo htmlspecialchars_decode($product->get_price_html());
                                    else :
                                        if ( $product->tax_display_cart == 'excl' ) :
                                            echo woocommerce_price($product->get_price_excluding_tax()). ' per m <small class="woocommerce-price-suffix">' . __('excl. Vat', 'pvc') . '</small>';
                                        else :
                                            echo woocommerce_price($product->get_price_including_tax()). ' per m <small class="woocommerce-price-suffix">' . __('incl. Vat', 'pvc') . '</small>';
                                        endif;
                                    endif;
                                    ?>
                                    </div>
...
?>  
        ...
        <div class="daily-deal col-sm-6 col-xs-12">
          <h3 class="deal-title">
            <?php echo htmlspecialchars_decode($pvc_Options['home_daily_deal_title']);?>
          </h3>

          <p>
             <?php echo htmlspecialchars_decode($pvc_Options['home_daily_deal_text']);?>
          </p>
          <div class="box-timer">
            <div class="countbox_1 timer-grid"  data-time="<?php echo esc_html($sales_price_date_to) ;?>">
          </div>
          </div>
          <a target="_self" href="<?php echo !empty($pvc_Options['daily_deal_url']) ? esc_url($pvc_Options['daily_deal_url']) : '#' ?>" class="link">
            <?php esc_attr_e('Shop now','pvc'); ?>
          </a>
         </div>
