<?php
/**
 * Abstract Pvc Widget Class
 *
 * @author      PvcThemes
 * @category    Widgets
 * @package     PvcThemes/Abstracts
 * @version     2.1.0
 * @extends     WP_Widget
 */
abstract class magikPvc_Widget extends WP_Widget {
				...
				case "icon":
					...
					<div style="margin: 13px 0">
						...
						<div>
							...
							<input style="float: right"  title="<?php echo esc_attr_e('Click to browse icon','pvc') ?>" class="browse-icon button-secondary" type="button" value="<?php echo esc_attr_e('Browse...','pvc') ?>" />
...
if (!function_exists('magikPvc_post_thumbnail')) {
    function magikPvc_post_thumbnail($size = '') {
        $html = '';
        $prefix = 'magikPvc_';
        $width = '';
        $height = '';
        global $magikPvc_image_size;
        if (isset($magikPvc_image_size[$size])) {
            $width = $magikPvc_image_size[$size]['width'];
            $height = $magikPvc_image_size[$size]['height'];
        }
        ...
        switch(get_post_format()) {
            ...
            case 'image' :
                ...
                $image = magikPvc_get_image($args);

                if (!$image) break;
                $html = magikPvc_get_image_hover($image,$size, get_permalink(), the_title_attribute('echo=0'),get_the_ID());
                break;
            case 'gallery':
                $images = array();
                if (count($images) > 0) {
                    ...
                    foreach ($images as $image) {
                        ...
                        if (!empty($image_src)) {
                            $html .= magikPvc_get_image_hover($image_src,$size, get_permalink(), the_title_attribute('echo=0'),get_the_ID(),1);
                        }
                    }
                    $html .= '</div>';
                } else {
            ...
            case 'video':
                ...
                if (!is_single()) {
                    ...
                    if (!$image) {
                        ...
                        $video = $video[0];
                        if (filter_var($video, FILTER_VALIDATE_URL)) {
                            $html .= magikPvc_get_video_hover($image, get_permalink(), the_title_attribute('echo=0'), $video);
                        } else {
                            $html .= '<div class="embed-responsive embed-responsive-16by9 embed-responsive-' . $size . '">';
                            $html .= $video;
                            $html .= '</div>';
...
GET POST IMAGE
================================================== */
if (!function_exists('magikPvc_get_image')) {
    function magikPvc_get_image($args) {
        $default = apply_filters(
            'magikPvc_get_image_default_args',
            array(
                'post_id'  => get_the_ID(),
                'size'    => '',
                'width'    => '',
        ...
        $image_cache = wp_cache_get( $args['post_id'], 'magikPvc_get_image' );
        ...
        if ( empty( $image_cache[$key] ) ) {
            ...
            if ((!isset($image_src) || empty($image_src))  && $args['meta_key']) {
                $post_thumbnail_id = magikPvc_get_post_meta( $args['post_id'], $args['meta_key'], true );
            ...
            wp_cache_set( $args['post_id'], $image_cache, 'magikPvc_get_image' );
        } else {
            $image_src = $image_cache[$key];
        }
        $image_src = apply_filters( 'magikPvc_get_image', $image_src, $args );
        return $image_src;
    }
}
...
GET IMAGE HOVER
================================================== */
if (!function_exists('magikPvc_get_image_hover')) {
    function magikPvc_get_image_hover($image,$size, $url, $title, $post_id,$gallery = 0) {
        $attachment_id = magikPvc_get_attachment_id_from_url($image);
...
if (!function_exists('magikPvc_get_video_hover')) {
    function magikPvc_get_video_hover($image, $url, $title, $video_url) {
        return sprintf('<div class="entry-thumbnail">
...
================================================== */
if (!function_exists('magikPvc_get_attachment_id_from_url')) {
    function magikPvc_get_attachment_id_from_url($attachment_url = '') {
        global $wpdb;
        $attachment_id = false;
...
GET POST META
================================================== */
if ( !function_exists( 'magikPvc_get_post_meta' ) ) {
	function magikPvc_get_post_meta( $id, $key = "", $single = false ) {
